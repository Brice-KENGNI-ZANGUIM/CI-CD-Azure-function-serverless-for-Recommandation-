...
########################################################################################
@st.cache(suppress_st_warning=True)
def prediction(model, data, articles_metadata, article_embedding_80, user_id, n) :
    ...
    if "Cb" == model :
        ...
        #########    Génération des recommandation d'article et ajout à la liste des recommandations      #########
        ###########################################################################################################
        return distance_similarité( user_id, _data, _article_embedding_80, _articles_metadata, n = n ).reset_index(drop=True)[["article_id"]] 
    elif "Cosinus" == model :
        ###########################################################################################################
        #########    Génération des recommandation d'article et ajout à la liste des recommandations      #########
        ###########################################################################################################
